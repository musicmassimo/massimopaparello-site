...
interface TopNavProps {
  /** "fixed" (default) pins the bar to the viewport top, for use across
   *  every inner page. "inline" renders it in normal flow so it can sit
   *  inside a centered stack — e.g. layered under the name on the homepage
   *  hero, Hudson-Freeman-style. */
  layout?: "fixed" | "inline";
}

const TopNav = ({ layout = "fixed" }: TopNavProps) => {
  ...
  return (
    <nav
      ...
      className={
        layout === "fixed"
          ? "fixed top-0 left-0 right-0 flex justify-center items-center h-[44px] z-50"
          : "relative flex justify-center items-center h-[44px] z-50"
      }
    >
